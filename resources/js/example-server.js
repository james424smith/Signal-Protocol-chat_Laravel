...
function receiveKeys(req, res) {
    ...
    let storageKey =
        reqObj.registrationId.toString() + reqObj.deviceId.toString();
...
function sendKeys(req, res) {
    ...
    if (storageMap[storageKey]) {
        if (storageMap[storageKey].preKeys.length !== 0) {
            ...
            responseObject.preKey =
                responseObject.preKeys[responseObject.preKeys.length - 1];
...
function storeIncomingMessage(req, res) {
    let reqObj = req.body;
    let messageStorageKey =
        reqObj.messageTo.registrationId.toString() +
        reqObj.messageTo.deviceId.toString() +
        reqObj.messageFrom.registrationId.toString() +
        reqObj.messageFrom.deviceId.toString();
    ...
    if (messageStorageMap[messageStorageKey]) {
        if (storageMap[reqObj.messageFromUniqueId]) {
            ...
            responseObject.messageFrom = {
                registrationId:
                    storageMap[reqObj.messageFromUniqueId].registrationId,
